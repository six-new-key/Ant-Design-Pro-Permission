      ...
      <a-card :bordered="false" class="search-card" v-show="searchVisible">
        <a-form layout="inline" :model="searchForm" class="search-form-compact">
          <a-form-item name="username">
            <a-input v-model:value="searchForm.username" placeholder="请输入登录账号" allow-clear style="width: 180px" @pressEnter="handleSearch" />
          </a-form-item>
          <a-form-item name="ipaddr">
            <a-input v-model:value="searchForm.ipaddr" placeholder="请输入登录IP" allow-clear style="width: 180px" @pressEnter="handleSearch" />
          </a-form-item>
          <a-form-item name="status">
            <DictSelect
              v-model:value="searchForm.status"
              dict-type="login_status"
              placeholder="请选择登录状态"
              allow-clear
              value-type="number"
              style="width: 180px"
    ...
    <a-card :bordered="false">
      <template #title>
        <div class="table-header-actions">
          <a-space :size="12">
            <a-button type="primary" danger :disabled="selectedRowKeys.length === 0" @click="handleBatchDelete" v-permission.disable="'system:login_log:remove'">
          ...
          <a-space :size="12">
            ...
            <a-dropdown placement="bottomRight">
              <template #overlay>
                <a-menu>
                  <a-menu-item key="clean" @click="handleClean" v-if="hasPermission('system:login_log:clean')">
                    <ClearOutlined />
                    <span style="margin-left: 8px;">清空日志</span>
                  </a-menu-item>
                  <a-menu-item key="export" @click="handleExport" v-if="hasPermission('system:login_log:export')">
                    <DownloadOutlined />
...
import { getLoginLogList, deleteLoginLog, cleanLoginLog, exportLoginLog } from '@/api/logininfor'
...
const columns = [
  ...
  { title: '登录IP', dataIndex: 'ipaddr', key: 'ipaddr', width: 150 },
  ...
  { title: '提示消息', dataIndex: 'msg', key: 'msg', width: 200, ellipsis: true },
...
const loadData = async () => {
  ...
  try {
    const params = {
      ...
      ipaddr: searchForm.ipaddr || undefined,
...
const handleBatchDelete = () => {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${selectedRowKeys.value.length} 条登录日志吗？此操作不可撤销。`,
    okText: '确定删除',
    cancelText: '取消',
    okType: 'danger',
    centered: true,
