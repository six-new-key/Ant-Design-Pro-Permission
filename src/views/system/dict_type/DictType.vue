    ...
    <a-card :bordered="false" style="margin-bottom: 16px">
      <a-form layout="inline" :model="queryForm">
        <a-form-item label="字典名称">
          <a-input v-model:value="queryForm.name" placeholder="请输入字典名称" allow-clear style="width: 200px" />
        </a-form-item>
        <a-form-item label="字典类型">
          <a-input v-model:value="queryForm.type" placeholder="请输入字典类型" allow-clear style="width: 200px" />
        </a-form-item>
...
// 表格列定义
const columns = [
  { title: '字典ID', dataIndex: 'id', key: 'id', width: 80 },
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await getDictTypeList(pagination.current, pagination.pageSize, params)
    if (res.code === 200) {
